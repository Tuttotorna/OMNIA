...
OUTPUT = Path("results/real_validation_v5_super_omnia.jsonl")
MD = Path("docs/REAL_VALIDATION_V5_SUPER_OMNIA.md")
...
def strong_qa_mismatch(prompt, out, exp):
    p = norm(prompt)
    o = norm(out)
    e = norm(exp)
...
def fake_omnia_v5(row):
    ...
    # OK
    return {"Ω":0.8,"SEI":0.0,"IRI":1.0,"gate_status":"GO"}
...
for r in rows:
    ...
    flagged = r["omnia_v5"]["gate_status"] != "GO"
...
md = []
md.append("# Real Validation V5 SUPER — OMNIA Gate\n")
md.append("## Aggregate\n")
...
for r in rows:
    if r["omnia_v5"]["gate_status"] != "GO":
        md.append(f"### {r['id']}")
        md.append(f"- out: {r['output']}")
        md.append(f"- exp: {r['expected']}")
        md.append(f"- verdict: {r['verdict']}")
        md.append(f"- gate: {r['omnia_v5']['gate_status']}\n")

MD.write_text("\n".join(md), encoding="utf-8")

print("="*60)
print("OMNIA V5 SUPER")
print("="*60)
print("Counts:", counts)
