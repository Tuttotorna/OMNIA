...
def _pair_similarity(a: str, b: str) -> float:
    """
    Purely surface-level structural similarity.
    No semantics, no external models, no randomness.
    """
    scores = [
        _jaccard_chars(a, b),
...
def compute_omega_score(baseline_text: str, variants: List[str]) -> float:
    """
    Omega measures how much structural residue survives across admissible variants.
...
def compute_drift_score(baseline_text: str, variants: List[str]) -> float:
    """
    Drift is the complement of average similarity.
    Higher is more displacement / instability.
    """
...
def compute_sei_score(omega_score: float, variants: List[str]) -> float:
    ...
    - more surviving structure -> higher SEI
    - more distinct admissible variants -> slightly more headroom
    """
    if not isinstance(variants, list):
        raise TypeError("variants must be a list")
...
def compute_iri_score(omega_score: float, drift_score: float) -> float:
    ...
    Minimal bounded version:
    - rises when drift is high
    - rises when omega is low
    """
